...
// v8 paths are [gx,gz,cy] triples; these 2D scenarios pin cy === 0 throughout.
const cells = (path) => {
  const out = []
  for (let i = 0; i < path.length; i += 3) {
    expect(path[i + 2]).toBe(0)
...
describe('pathfind: findPath', () => {
  it('returns the optimal straight line in open space', () => {
    ...
    const path = findPath(cm, wc(2), wc(2), 0, wc(10), wc(2), 0, { collapse: false })
  ...
  it('uses no diagonal moves', () => {
    const cm = mockCM(new ChunkData(0, 0, 0, 0))
    const path = findPath(cm, wc(2), wc(2), 0, wc(9), wc(6), 0, { collapse: false })
    const c = cells(path)
  ...
  it('detours around a full wall to the single gap', () => {
    ...
    const path = findPath(cm, wc(2), wc(7), 0, wc(8), wc(7), 0)
    expect(path).not.toBeNull()
    const c = cells(path)
    let span = 0
  ...
  it('routes straight through a doorway gap', () => {
    ...
    const cm = mockCM(data)
    const path = findPath(cm, wc(2), wc(7), 0, wc(8), wc(7), 0, { collapse: false })
    expect(path).not.toBeNull()
    const c = cells(path)
  ...
  it('returns null when the start is boxed in', () => {
    ...
    data.setH(4, 5, 1)
    const cm = mockCM(data)
    expect(findPath(cm, wc(4), wc(4), 0, wc(10), wc(10), 0)).toBeNull()
  })
  ...
  it('returns null when the target is boxed in', () => {
    ...
    data.setH(9, 10, 1)
    const cm = mockCM(data)
    expect(findPath(cm, wc(2), wc(2), 0, wc(9), wc(9), 0)).toBeNull()
  })

  it('respects the maxNodes budget cap', () => {
    const cm = mockCM(new ChunkData(0, 0, 0, 0))
    expect(findPath(cm, wc(2), wc(2), 0, wc(12), wc(12), 0, { maxNodes: 5 })).toBeNull()
  })
  ...
  it('bails immediately when the target is beyond the leash', () => {
    ...
    expect(findPath(cm, wc(2), wc(2), 0, wc(10), wc(2), 0, { leash: 3, maxNodes: 1e6 })).toBeNull()
  })
  ...
  it('retargets a column target to an open neighbour, or null when enclosed', () => {
    ...
    const cm = mockCM(data)
    const path = findPath(cm, wc(2), wc(2), 0, wc(9), wc(9), 0)
    expect(path).not.toBeNull()
    const c = cells(path)
  ...
  it('is deterministic across identical calls', () => {
    ...
    data.setV(6, 9, 0)
    const cm = mockCM(data)
    const a = findPath(cm, wc(1), wc(1), 0, wc(11), wc(2), 0)
    const b = findPath(cm, wc(1), wc(1), 0, wc(11), wc(2), 0)
  ...
  it('reuses the caller-supplied out buffer', () => {
    const cm = mockCM(new ChunkData(0, 0, 0, 0))
    const out = []
    const r1 = findPath(cm, wc(2), wc(2), 0, wc(7), wc(2), 0, { out, collapse: false })
    expect(r1).toBe(out)
    expect(cells(out).length).toBe(6)
    const r2 = findPath(cm, wc(2), wc(2), 0, wc(5), wc(2), 0, { out, collapse: false })
    expect(r2).toBe(out)
...
describe('pathfind: makeCanPass / edgeOpen', () => {
  it('agrees with the edge rule on a wall and an open edge', () => {
    ...
    expect(edgeOpen(cm, 4, 7, 0, 0)).toBe(false)
    expect(edgeOpen(cm, 5, 7, 0, 0)).toBe(true)
  })
})
...
describe('pathfind: followPath', () => {
  it('string-pulls past a colinear waypoint and converges to the target', () => {
    const cm = mockCM(new ChunkData(0, 0, 0, 0))
    const path = [2, 5, 0, 5, 5, 0, 8, 5, 0] // colinear middle waypoint (5,5)
    const ent = { pos: { x: wc(2), y: 0, z: wc(5) }, cy: 0 }

    const first = followPath(cm, ent, path, 0, 1.0)
    expect(first.i).toBe(2) // jumped straight to the last waypoint (clear LOS)

    let r = first
    for (let k = 0; k < 80 && !r.done; k++) r = followPath(cm, ent, path, r.i, 1.0)
    expect(r.done).toBe(true)
    expect(Math.abs(ent.pos.x - wc(8))).toBeLessThan(CELL)
    expect(Math.abs(ent.pos.z - wc(5))).toBeLessThan(0.5)
  })
})
